    ...
    private void Attack2_D()
    {
        if (lasering)
        {
            ...
            if (laserTimer > 0)
            {
                ...
                attackD.transform.gameObject.SetActive(true);
                //如果击中
                if (hitInfo.collider != null)
                {
                    //射线起始位置
                    attackD.SetPosition(0, Attack2_DPoint.position);
                    //射线末端位置
                    attackD.SetPosition(1, hitInfo.point);
                    //击中特效
                ...
                else
                {
                    ...
                    attackD.SetPosition(0, Attack2_DPoint.position);
                    //射线末端位置
                    attackD.SetPosition(1, Attack2_DPoint.up * 100);
                    //击中特效
                ...
                {
                    Attack2_DPoint.rotation = Quaternion.Euler(0, 0, start1 -= 10 * Time.deltaTime);
                }
                else if (ang > 0 &&
                        ang <= 50 &&
                        player.transform.position.x < transform.position.x)
                {
                    Attack2_DPoint.rotation = Quaternion.Euler(0, 0, start2 -= 10 * Time.deltaTime);
                }
                else if (ang > 0 &&
                    ang <= 50 &&
                    player.transform.position.x > transform.position.x) 
                {
                    Attack2_DPoint.rotation = Quaternion.Euler(0, 0, start4 += 10 * Time.deltaTime);
                }
                else if (ang > 50 &&
                    player.transform.position.x > transform.position.x)
                {
                    Attack2_DPoint.rotation = Quaternion.Euler(0, 0, start3 += 10 * Time.deltaTime);
                }
            }
            else
            {
                ...
                LaserHurtVFX.SetActive(false);
                attackD.transform.gameObject.SetActive(false);
                start1 = 140;
                start2 = 50;
                start3 = -140;
                start4 = -50;
